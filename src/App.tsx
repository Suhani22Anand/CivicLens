...
import { Sun, Moon, Languages, User, ShieldCheck, Sparkles, HelpCircle, Layers, PlusCircle, LayoutDashboard, Database, HelpCircle as HelpIcon, Activity, Bell, X, Users, ArrowRight, Lock, Clock, Home, ChevronDown } from 'lucide-react';
...
export default function App() {
  // Navigation State
  const [activeTab, setActiveTab] = useState<'HomePage' | 'Home' | 'AllPosts' | 'PostIssue' | 'Dashboard' | 'Profile' | 'CommunityGroups' | 'Admin'>('HomePage');
  ...
  // Custom Dropdowns States (Beautifying language and persona switchers)
  ...
  const handleGenerateOTP = async (e: React.FormEvent) => {
    ...
    setOtpCountdown(60);
    // Robust client-side email format regex validation
    const emailRegex = /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+$/;
    if (!emailRegex.test(regContact.trim())) {
      alert('Please enter a valid email address (e.g., name@example.com).');
      return;
    }

    // Check role constraints if signing up as Municipal Officer
    if (regType === 'Municipal Officer') {
      const lowerEmail = regContact.trim().toLowerCase();
      if (!lowerEmail.endsWith('@gov.in') && !lowerEmail.endsWith('@nic.in')) {
        alert('Municipal Officer access is restricted to verified government domains (@gov.in or @nic.in). Please enter a valid municipal authority email.');
        return;
      }
    }
  ...
  const isLoggedIn = !!customRegisteredUser;
  ...
  useEffect(() => {
    ...
    return () => {
      delete (window as any).triggerLoginPortal;
    };
  }, []);
  ...
  const handleUpdateProfile = (updatedUser: UserProfile) => {
    setCustomRegisteredUser(updatedUser);
  };
  ...
  // Dynamic Real-time Email & Domain Validation
  const emailVal = regContact.trim();
  const emailRegex = /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+$/;
  const isEmailFormatValid = emailRegex.test(emailVal);
  const isGovDomain = emailVal.toLowerCase().endsWith('@gov.in') || emailVal.toLowerCase().endsWith('@nic.in');
  
  let emailStatusText = '';
  let emailStatusColor = 'text-slate-400 dark:text-slate-500';
  let isSubmitDisabled = false;

  if (emailVal === '') {
    emailStatusText = 'Please enter your active email address.';
    emailStatusColor = 'text-slate-400 dark:text-slate-500';
  } else if (!isEmailFormatValid) {
    emailStatusText = '⚠️ Enter a complete, valid email format (e.g. name@domain.com).';
    emailStatusColor = 'text-amber-600 dark:text-amber-400 font-semibold';
    isSubmitDisabled = true;
  } else {
    if (regType === 'Municipal Officer') {
      if (!isGovDomain) {
        emailStatusText = '❌ Municipal access restricted to government domains (@gov.in or @nic.in).';
        emailStatusColor = 'text-rose-600 dark:text-rose-400 font-extrabold animate-pulse';
        isSubmitDisabled = true;
      } else {
        emailStatusText = '✅ Verified Government Domain! Ready to receive OTP.';
        emailStatusColor = 'text-emerald-600 dark:text-emerald-400 font-extrabold';
      }
    } else {
      emailStatusText = '✅ Valid email format! Ready to receive OTP.';
      emailStatusColor = 'text-emerald-600 dark:text-emerald-400 font-extrabold';
    }
  }
    ...
    <div className={`min-h-screen transition-colors duration-300 font-sans ${theme === 'dark' ? 'bg-slate-900 text-slate-100' : 'bg-[#F1F5F9] text-slate-800'}`}>
      ...
      <div className="bg-slate-800 text-slate-300 text-xs py-3.5 px-6 border-b border-slate-700 flex flex-wrap justify-between items-center gap-4 shadow-md font-sans">
        ...
        <div className="flex flex-wrap items-center gap-4">
          {/* Persona Switcher (Beautified Custom Dropdown) */}
          <div className="relative">
            <button
              onClick={() => {
                setRoleDropdownOpen(!roleDropdownOpen);
                setLangDropdownOpen(false); // Close other dropdown
              }}
              className="flex items-center gap-2 bg-slate-900 hover:bg-slate-950 px-3 py-1.5 rounded-lg border border-slate-700/80 text-white font-bold text-xs focus:outline-none cursor-pointer transition-colors"
            >
              <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wide">
                {getTranslation(lang, 'roleSwitcher')}:
              </span>
              <span className="text-emerald-400 font-extrabold">
                @{sampleUsers[currentUserIdx].username} ({currentUserIdx === 0 ? getTranslation(lang, 'roleCitizen') : currentUserIdx === 1 ? getTranslation(lang, 'roleNGO') : getTranslation(lang, 'roleOfficer')})
              </span>
              ...
              {roleDropdownOpen && (
                <>
                  ...
                  <motion.div
                    ...
                    className="absolute right-0 mt-2 w-72 rounded-2xl bg-slate-900 border border-slate-700 shadow-2xl z-50 py-2.5 overflow-hidden font-sans"
                  >
                    <div className="px-3.5 pb-2 border-b border-slate-800 text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                      Select Demo Role
                    </div>
                    {sampleUsers.map((u, idx) => {
                      const active = currentUserIdx === idx;
                      const roleLabel = idx === 0 ? getTranslation(lang, 'roleCitizen') : idx === 1 ? getTranslation(lang, 'roleNGO') : getTranslation(lang, 'roleOfficer');
                      return (
                        <button
                          key={idx}
                          onClick={() => {
                          ...
                          className={`w-full text-left px-4 py-2.5 flex flex-col gap-0.5 hover:bg-slate-800/80 transition-colors ${active ? 'bg-slate-800 text-emerald-400' : 'text-slate-300'}`}
                        >
                          <span className={`font-black text-xs ${active ? 'text-emerald-400' : 'text-white'}`}>
                            @{u.username}
                          </span>
          ...
          {/* Language Selector (Beautified Custom Dropdown) */}
          <div className="relative">
            <button
              onClick={() => {
              ...
              className="flex items-center gap-1.5 bg-slate-900 hover:bg-slate-950 px-2.5 py-1.5 rounded-lg border border-slate-700/80 text-white font-bold text-xs focus:outline-none cursor-pointer transition-colors"
            >
              <Languages className="w-3.5 h-3.5 text-slate-400" />
              <span>
                {lang === 'English' ? 'English' : lang === 'Hindi' ? 'हिन्दी' : lang === 'Spanish' ? 'Español' : lang === 'Tamil' ? 'தமிழ்' : 'मराठी'}
              </span>
              ...
              {langDropdownOpen && (
                <>
                  ...
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: 8, scale: 0.95 }}
                    transition={{ duration: 0.15 }}
                    className="absolute right-0 mt-2 w-52 rounded-2xl bg-slate-900 border border-slate-700 shadow-2xl z-50 py-2 overflow-hidden font-sans"
                  >
                    <div className="px-3.5 pb-2 border-b border-slate-800 text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                      Select Language
                    </div>
                    {([
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left px-4 py-2.5 font-bold text-xs hover:bg-slate-800/80 transition-colors ${active ? 'bg-slate-800 text-emerald-400' : 'text-slate-300'}`}
                        >
                          {opt.label}
                        </button>
                      );
      ...
      <header className="sticky top-0 z-40 backdrop-blur-md bg-white/95 dark:bg-slate-900/95 border-b border-slate-100 dark:border-slate-800 shadow-sm px-6 py-4 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <nav className="flex flex-wrap gap-1 md:gap-2">
          {([
            { id: 'HomePage', label: getTranslation(lang, 'navHomePage'), icon: <Home className="w-4 h-4 text-[#6b9661]" /> },
            { id: 'Home', label: getTranslation(lang, 'navHome'), icon: <Layers className="w-4 h-4 text-[#6b9661]" /> },
            { id: 'AllPosts', label: getTranslation(lang, 'navAllPosts'), icon: <Database className="w-4 h-4 text-[#6b9661]" /> },
            { id: 'PostIssue', label: getTranslation(lang, 'navPostIssue'), icon: <PlusCircle className="w-4 h-4 text-[#6b9661]" /> },
            { id: 'CommunityGroups', label: getTranslation(lang, 'navCommunityGroups'), icon: <Users className="w-4 h-4 text-[#6b9661]" /> },
            { id: 'Dashboard', label: getTranslation(lang, 'navDashboard'), icon: <LayoutDashboard className="w-4 h-4 text-[#6b9661]" /> },
            { id: 'Profile', label: getTranslation(lang, 'navProfile'), icon: <User className="w-4 h-4 text-[#6b9661]" /> },
            ...(!isLoggedIn ? [{ id: 'Login', label: getTranslation(lang, 'navLogin'), icon: <Lock className="w-4 h-4 text-[#6b9661] animate-pulse" /> }] : [])
          ] as any[]).map((item) => {
...
{`alter table registrations disable row level security;`}
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        <div className="space-y-8" id="main-content-wrapper">
          {/* active tab rendering */}
          {!isLoggedIn && activeTab !== 'HomePage' ? (
          <div className="flex flex-col items-center justify-center min-h-[450px] text-center p-8 rounded-3xl border border-[#4c6c9a]/30 shadow-sm max-w-2xl mx-auto space-y-6 my-10 relative overflow-hidden text-white" style={{ backgroundColor: '#1b283c' }}>
            <div className="absolute top-0 right-0 w-32 h-32 bg-[#6b9661]/5 rounded-full blur-2xl pointer-events-none"></div>
            <div className="absolute bottom-0 left-0 w-32 h-32 bg-[#4c6c9a]/5 rounded-full blur-2xl pointer-events-none"></div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-300 block font-bold uppercase tracking-widest">
                Please Sign Up / Login to view
              </p>
              <p className="text-sm text-slate-300 max-w-md mx-auto leading-relaxed">
                The "{activeTab === 'AllPosts' ? 'All Reports' : activeTab === 'CommunityGroups' ? 'Action Groups' : activeTab}" section is restricted to registered members of our active municipal consensus-building network.
              </p>
            </div>
            ...
            <button
              ...
              className="bg-gradient-to-r from-[#4c6c9a] to-[#6b9661] text-white px-8 py-3.5 rounded-2xl font-black text-xs transition-all hover:scale-[1.03] active:scale-95 shadow-md cursor-pointer flex items-center gap-2"
            >
              <Lock className="w-4 h-4" />
              <span>Sign Up or Login to Unlock All Sections</span>
            </button>
          </div>
        ) : (
          <>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-md overflow-y-auto" id="registration-portal-overlay">
          <div className="bg-white dark:bg-slate-800 rounded-3xl max-w-lg w-full p-8 border border-slate-200 dark:border-slate-700 shadow-2xl relative overflow-hidden text-left">
            ...
            <button
              ...
              className="absolute top-5 right-5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors cursor-pointer p-1 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700/50 z-10"
              title="Close Portal"
            >
              <X className="w-5 h-5" />
            </button>
                ...
                <form onSubmit={handleGenerateOTP} className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 px-4 py-2.5 rounded-xl text-xs font-semibold focus:outline-none focus:border-[#4c6c9a] text-slate-800 dark:text-slate-100"
                    />
                    <div className={`text-[10px] ${emailStatusColor} mt-1 leading-snug transition-colors duration-200`}>
                      {emailStatusText}
                    </div>
                  </div>
                  ...
                  <button
                    type="submit"
                    disabled={isSendingOTP || isSubmitDisabled}
                    className="w-full bg-[#4c6c9a] hover:bg-[#405c83] disabled:bg-slate-200 dark:disabled:bg-slate-800/80 disabled:opacity-50 disabled:text-slate-400 dark:disabled:text-slate-600 font-bold py-3 rounded-xl shadow text-xs transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                  >
                ...
                {otpMode === 'real' ? (
                  <div className="bg-emerald-500/10 border border-emerald-500/20 p-5 rounded-3xl text-left space-y-4">
                    <div className="space-y-1">
                      <p className="text-xs text-slate-700 dark:text-slate-200 leading-relaxed font-black">
                        Email dispatched to <span className="text-indigo-600 dark:text-indigo-400 font-bold">{regContact}</span>!
                      </p>
                      <p className="text-[11px] text-slate-500 dark:text-slate-400 leading-relaxed">
                        Please check your email inbox (including your spam folder) and enter the 6-digit confirmation code below.
                      </p>
                    </div>
